<template>
  <div class="w-full max-w-4xl mx-auto">
    <!-- 1. 未登入 LINE 帳號 -->
    <div v-if="!isLoggedIn" class="surface-card rounded-sm border border-border p-8 sm:p-10 mb-8 text-center space-y-3.5">
      <div class="inline-flex h-10 w-10 items-center justify-center rounded-full bg-muted/40 text-muted-foreground mb-1">
        <Lock class="h-5 w-5" />
      </div>
      <div>
        <h3 class="text-base font-bold text-foreground">查詢預約請先登入 LINE 帳號</h3>
        <p class="mt-1 text-xs text-muted-foreground max-w-sm mx-auto leading-relaxed">
