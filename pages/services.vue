<template>
  <div>
    <PageHero eyebrow="Services" title="四項服務，一套完整邏輯" desc="從徒手處理到訓練課表，依你的評估結果與運動週期組合出最適合的方案。" />

    <Section>
      <div class="space-y-20">
        <article v-for="(service, i) in SERVICES" :key="service.slug" :id="service.slug" class="grid items-center gap-10 lg:grid-cols-2">
          <img
            :src="IMAGES[service.slug]"
            :alt="ALTS[service.slug]"
            width="1200"
            height="900"
            loading="lazy"
            :class="cn('w-full rounded-sm border border-border object-cover', i % 2 === 1 && 'lg:order-2')"
          />
          <div>
            <span class="eyebrow">0{{ i + 1 }}</span>
            <h2 class="mt-3 text-3xl sm:text-4xl">{{ service.title }}</h2>
            <p class="mt-4 text-base leading-relaxed text-muted-foreground">
              {{ service.short }}
            </p>
            <ul class="mt-6 space-y-3 text-sm text-muted-foreground">
              <li v-for="p in service.points" :key="p" class="flex gap-3">
    ...
    <Section tone="ink">
      <SectionHeading center eyebrow="Not Sure?" title="不確定該從哪一項開始？" desc="第一次來訪都會先進行完整評估，我們會依結果建議最適合的方案。" />
...
useHead({
  ...
  link: [{ rel: "canonical", href: "https://www.kse-release.com.tw/services" }],
})
</script>
